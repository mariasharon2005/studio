...
export default function Logo({ className, size = "md" }: LogoProps) {
  ...
  return (
    <div className={cn("relative flex items-center justify-center group", sizeMap[size], className)}>
      {/* 3D Depth Layers */}
      <div className="absolute inset-0 bg-primary/20 rounded-full blur-2xl group-hover:bg-primary/40 transition-all duration-1000 animate-pulse-slow"></div>
      
      <svg 
        viewBox="0 0 100 100" 
        className="w-full h-full drop-shadow-[0_0_20px_hsla(var(--primary),0.5)] transition-all duration-500"
      >
        <defs>
        ...
        {/* Connectivity Grid (Stealth Wireframe in Ghost Mode) */}
